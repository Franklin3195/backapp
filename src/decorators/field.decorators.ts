...
interface IClassFieldArrayOptions extends IFieldOptions {
  min?: number;
  max?: number;
}
...
// eslint-disable-next-line @typescript-eslint/ban-types
export function ClassFieldArray<TClass extends object>(
  getClass: () => TClass,
  options: Omit<ApiPropertyOptions, 'type'> & IClassFieldArrayOptions = {},
): PropertyDecorator {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const classValue = getClass() as any;

  if (!classValue) {
    throw new Error('ClassFieldArray: recursive class definition');
  }

  const decorators = [
    Type(() => classValue),
    ValidateNested({ each: true }), // Note the "each" here is always true for arrays
  ];
